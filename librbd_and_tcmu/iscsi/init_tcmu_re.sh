...
TARGET_NAME="iqn.2020-05.com.yujiang.iscsi-gw-jerasure"
...
# DELETE 
# curl --insecure --user xcuser:xcpassword -d disk=${POOL_NAME}/${RBD_NAME} -X PUT http://${CONTROL_GATEWAY_IP}:5000/api/clientlun/${TARGET_NAME}/${INITIATOR_NAME}
# curl --insecure --user xcuser:xcpassword -X DELETE http://${CONTROL_GATEWAY_IP}:5000/api/client/${TARGET_NAME}/${INITIATOR_NAME}
# curl --insecure --user xcuser:xcpassword -d disk=${POOL_NAME}/${RBD_NAME} -X DELETE http://${CONTROL_GATEWAY_IP}:5000/api/targetlun/${TARGET_NAME}
